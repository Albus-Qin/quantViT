...
from .ptq import QAct, QConv2d, QIntLayerNorm, QIntSoftmax, QLinear, QPreSoftmaxAct, QGELU, QPostSoftmaxAct,QPostGELUAct
...
class Attention(nn.Module):
    ...
    def __init__(self,
        ...
        self.qact3 = QAct(quant=quant,
                          ...
                          observer_str=cfg.OBSERVER_A,
                          quantizer_str=cfg.QUANTIZER_A)
        self.qact_attn1 = QPreSoftmaxAct( num_heads = num_heads,
                               quant=quant,
                               calibrate=calibrate,
                               bit_type=cfg.BIT_TYPE_A,
                               calibration_mode=cfg.CALIBRATION_MODE_A,
                               observer_str=cfg.OBSERVER_A,
                               quantizer_str=cfg.QUANTIZER_A)
        self.qact_attn2 = QPostSoftmaxAct(
                                         quant=quant,
                                         calibrate=calibrate,
    ...
    def forward(self, x):
        ...
        attn = self.log_int_softmax(attn, self.qact_attn1.scales)
...
class VisionTransformer(nn.Module):
    ...
    def model_quant(self):
        for m in self.modules():
            if type(m) in [QAct, QConv2d, QIntLayerNorm, QIntSoftmax, QLinear, QPreSoftmaxAct, QPostSoftmaxAct, QGELU, QPostGELUAct]:
                m.quant = True
